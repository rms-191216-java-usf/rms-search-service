...
public class ETLServiceTest {
  ...
  public void testGetCampusDtoByIdWithValidInput() {
    ...
    List<RoomStatus> roomStatuses = Arrays.asList(new RoomStatus("1", true, true, true, "1/1/19", 1, "Good"));
    List<Room> rooms = Arrays.asList(new Room("1", "123", 25, true, roomStatuses, 1, workOrders, resourceMetadata));
    List<Building> buildings = Arrays.asList(new Building("1", "Muma", "BSN", address, 1, amenities, rooms, resourceMetadata));
    Campus campus = new Campus("1", "USF", "USF", address, 1, 1, 1, buildings, employees, resourceMetadata);
    EmployeeDto empDto = new EmployeeDto(1, "Blake", "Dunn", "Email@email.com", "Baller", Department.TRAINING, new ResourceMetadataDto());
    ResourceMetadataDto rMDto= new ResourceMetadataDto(empDto, "1/1/20", empDto, "1/1/20", empDto);
    List<RoomStatusDto> roomStatusDto = Arrays.asList(new RoomStatusDto("1", true, true, true, "1/1/19", empDto, "Good"));
    List<RoomDto> roomDto = Arrays.asList(new RoomDto("1", "123", 25, true, roomStatusDto, rMDto));
    List<BuildingDto> buildingDto = Arrays.asList(new BuildingDto("1", "Muma", "BSN", address, empDto, amenities, roomDto, rMDto));
    CampusDto mockCampusDto = new CampusDto("1", "USF", "USF", address, empDto, empDto, empDto, buildingDto, Arrays.asList(empDto), rMDto);
    when(mockCampusClient.getCampusById("1")).thenReturn(campus);
    CampusDto expectedResult = (CampusDto) when(sut.getCampusObjects(campus)).thenReturn(mockCampusDto);
